...
describes what users get. Pick a format with `-o`, or let usaspending choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
usaspending <command> -o table   # aligned columns for reading
usaspending <command> -o jsonl   # one JSON object per line, for piping
usaspending <command> -o json    # a single JSON array
usaspending <command> -o csv     # spreadsheet friendly
usaspending <command> -o tsv     # tab-separated
usaspending <command> -o url     # just the URL column
usaspending <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
usaspending <command> --fields id,title,url
```
...
```bash
usaspending <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
usaspending <command>            # a table, because this is a terminal
usaspending <command> | wc -l    # JSONL, because this is a pipe
```
